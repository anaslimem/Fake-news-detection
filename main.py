...
train_vectors_df1 = np.array([document_vector(tokens, word_vectors) for tokens in train_df1['text']])
train_vectors_df2 = np.array([document_vector(tokens, word_vectors) for tokens in train_df2['text']])
X_train = np.vstack([train_vectors_df1, train_vectors_df2])

test_vectors_df1 = np.array([document_vector(tokens, word_vectors) for tokens in test_df1['text']])
test_vectors_df2 = np.array([document_vector(tokens, word_vectors) for tokens in test_df2['text']])
X_test = np.vstack([test_vectors_df1, test_vectors_df2])

y_train = [0] * len(train_df1) + [1] * len(train_df2)  # 0 for fake, 1 for true
...
def predict_news(news_text):
    ...
    prediction = svm.predict(np.atleast_2d(vector))
    if prediction[0] == 0:
        return "Fake News"
    else:
        return "True News"
